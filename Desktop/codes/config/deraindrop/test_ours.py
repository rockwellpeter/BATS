...
parser.add_argument("-opt", type=str, default="config/deraining/options/test/ours.yml", help="Path to options YMAL file.")
...
for test_loader in test_loaders:
    ...
    for i, test_data in enumerate(test_loader):
        ...
        model.test_visual_x0(sde=sde,current_step=-1,name="x0-multistep-unrolled-prob-alpha")
        toc = time.time()
        ...
        if need_GT:
            ...
            sr_dir = 'temp_sr_unrolled_ori_derain_single'
            gt_dir = 'temp_gt_unrolled_ori_derain_single'
            ...
            if len(gt_img.shape) == 3:
                if gt_img.shape[2] == 3:  # RGB image
                    ...
                    logger.info(
                        "img{:3d}:{:15s} - PSNR: {:.6f} dB; SSIM: {:.6f}; LPIPS: {:.6f}; FID: {:.6f}; PSNR_Y: {:.6f} dB; SSIM_Y: {:.6f}.".format(
                            i, img_name, psnr, ssim, lp_score, fid, psnr_y, ssim_y
                        )
                    )
            else:
                logger.info(
