...
def main():
    ...
    parser.add_argument("-opt", type=str,  default="config/inpainting/options/train/ours.yml",help="Path to option YMAL file.")
    parser.add_argument(
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0-7)
        if resume_state is None:
            ...
            os.system("rm ./log_derain")
            os.symlink(os.path.join(opt["path"]["experiments_root"], ".."), "./log_derain")
        ...
        if opt["use_tb_logger"] and "debug" not in opt["name"]:
            ...
            tb_logger = SummaryWriter(log_dir="log_derain/{}/tb_logger/".format(opt["name"]))
    else:
        util.setup_logger(
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            # 一定要k=0，确保能取到最后几步
            timesteps, states, noises = sde.generate_random_states_ours(x0=GT, mu=LQ, type='alpha')
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for idx, val_data in enumerate(val_loader):
                    ...
                    # model.test(sde)
                    model.test_visual_x0(sde=sde,current_step=current_step)
            ...
            if current_step % opt["logger"]["save_checkpoint_freq"] == 0:
                # logger.info("Saving latest training states.")
                # model.save_training_state(epoch, current_step,save_name='latest',psnr=avg_psnr)
                pass
