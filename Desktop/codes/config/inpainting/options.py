...
import sys
import math
sys.path.append("/home/proj/image-restoration-sde-main/codes")
import yaml
...
def parse(opt_path, is_train=True):
    ...
    if is_train:
        ...
        experiments_root = osp.join(
            opt["path"]["root"], "experiments", config_dir, opt["name"] + "_" + timestamp
             )
        ...
        opt["path"]["results_root"] = osp.join(results_root, opt["name"]+ "_" + timestamp)
        opt["path"]["log"] = osp.join(results_root, opt["name"]+ "_" + timestamp)
        opt["path"]["results_root"] = osp.join(results_root, opt["name"]+ "_" + timestamp)
        opt["path"]["log"] = osp.join(results_root, opt["name"]+ "_" + timestamp)
