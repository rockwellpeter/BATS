...
def main():
    ...
    parser.add_argument("-opt", type=str,  default="config/sisr/options/train/ours.yml",help="Path to option YMAL file.")
    parser.add_argument(
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            timesteps, states, noises = sde.generate_random_states_ours(x0=GT, mu=LQ)

            model.feed_data(states, LQ, GT) # xt, mu, x0
            # model.optimize_parameters(current_step, timesteps, sde)
            model.optimize_parameters_x0_ours(current_step, timesteps, sde, noises)
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for idx, val_data in enumerate(val_loader):
                    ...
                    LQ = util.upscale(LQ, scale)
                    # #Set5
                    # GT_shape = val_data["GT"].shape
                    # h_gt = GT_shape[-2]
                    # w_gt = GT_shape[-1]
                    # LQ = util.upscale_size(LQ, (h_gt,w_gt))
                    ...
                    # model.test(sde)
                    model.test_visual(sde=sde,current_step=current_step)
