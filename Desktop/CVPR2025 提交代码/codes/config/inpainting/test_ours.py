...
parser.add_argument("-opt", type=str, default="config/inpainting/options/test/ours.yml", help="Path to options YMAL file.")
...
for test_loader in test_loaders:
    ...
    for i, test_data in enumerate(test_loader):
        ...
        GT = test_data["GT"]
        # LQ = test_data["LQ"]
        LQ = util.mask_to(GT, mask_root, mask_id=i%100)
        noisy_state = sde.noise_state(LQ)

        model.feed_data(noisy_state, LQ, GT)
        tic = time.time()
        # model.test(sde, save_states=True)
        model.test_visual_x0(sde=sde,current_step=-1)
        toc = time.time()
